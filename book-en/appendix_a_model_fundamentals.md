...
[Figure 22](#fig-appendix-block-reading) is that same block drawn for the direction each stage mixes,
...
>   position rather than by content, and drawn in [Figure 25](#fig-appendix-attention).
>
...
[Figure 23](#fig-appendix-softmax-shares) draws that same row three times, so the two jobs the exponential does and the normalising divide are visible as a change of shape rather than as a sentence.
...
[Figure 24](#fig-appendix-score-matrix) lays the mask over the scores it deletes, so the two
...
to look back, so they persist. That pair is the cache, and [Figure 25](#fig-appendix-attention) draws it
...
**Hardware application.** [Figure 26](#fig-appendix-dense-separable) sets the two shapes beside each other.
