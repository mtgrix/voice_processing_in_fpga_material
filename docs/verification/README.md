...
**One omission was closed on 2026-09-13, and it is worth naming because of how it hid.** The auditor never read the `unit` field except to exempt citation records from the traceability test, so relabelling V-01-13 from `GB/s` to `banana` left all seven gates green -- the arithmetic still recomputed, because no check compared the label to anything. `check_units` is now the sixth check: every spelling must be registered in `claims_lib.UNIT_TERMS`, every canonical must carry a kind, and no canonical may fold kilobits in with kilobytes, which is the one place where a case-insensitive normaliser would have created a worse defect than the one it fixed (V-01-11 counts bits, V-01-17 counts bytes). What still is not checked is whether a registered unit is the *correct* dimension for its value, because 39 of the 64 dimensional records carry prose values -- V-01-22 reads `23,616 Kb = 2,952 KiB = 3,022,848 B = 2.8828 MiB = 3.02 MB decimal` -- so that check requires splitting the field, not tightening it. Issue #25.
