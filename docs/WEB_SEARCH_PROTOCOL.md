...
| **T1-F** | **Not fetchable — there is no query for this row.** The achievable DSP fabric clock behind `V-07-03`'s 300 MHz. | *Search nothing.* A search returns a vendor “up to” figure from a marketing page, which launders an assumption into a sourced number. Carry it as a labelled assumption with a sensitivity table instead: at 500 MHz the pair becomes 65.0 and 130.0 OP/byte. | `plan-v2.md` §6.1, `V-07-03` |
