...
| NVIDIA documentation deep links | `docs.nvidia.com` behaves like `docs.amd.com` on the page routes: `…/nemo/user-guide/docs/en/stable/asr/results.html` was fetched 2026-09-14 and answers **HTTP 200 with a JavaScript shell and no document text**. | Quote the file the page is built from instead. NeMo's ASR documentation lives as plain text under `docs/source/`, so `raw.githubusercontent.com/NVIDIA-NeMo/Speech/<commit>/docs/source/asr/…` gives a quotable line with a line number. Pin the commit: a record that quotes line 11 of a moving file is true of one day's tree, which is why S026 … S029 are cited at one hash. |
| HuggingFace 401 | `huggingface.co/nvidia/stt_en_conformer_transducer_small` answers **HTTP 401 to an anonymous request** while the repository itself exists, measured 2026-09-14. Absence from a HuggingFace listing then proves nothing in either direction. | Never record "NVIDIA does not publish X" from a 401 or from an empty search listing. Record the search you ran as an `unresolved` record naming what was and was not reachable — `V-05-56` is the pattern — or hand the authenticated fetch to the owner, which is protocol rule 2's action, not yours. |
